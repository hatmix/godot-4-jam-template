...
The template UI is designed to work with keyboard, controller, and touch input as well as mouse. `Tab` and `shift+tab` move focus. Any controller input will grab focus except where this is prevented with `UiPage.prevent_joypad_focus_capture` which is used by the game ui page. G.U.I.D.E virtual joysticks have been added when a mobile browser is detected (see `src/ui/virtual_joysticks`). Virtual joystick support is experimental.
